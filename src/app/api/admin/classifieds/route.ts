...
export async function GET(req: NextRequest) {
  ...
  const items = await db.classified.findMany({ where, orderBy: { sortOrder: 'asc' } })
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const item = await db.classified.create({
      data: { issueId: body.issueId, heading: body.heading, body: body.body, sortOrder: body.sortOrder ?? 0 },
    })
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const body = await req.json()
    const item = await db.classified.update({
      where: { id: body.id },
      data: { heading: body.heading, body: body.body, sortOrder: body.sortOrder, issueId: body.issueId },
    })
